...
class DMem extends Module {
  ...
  val r_valid_address: Bool = (io.r_dmem_add.addr >= 0x2000.U) && (io.r_dmem_add.addr <= 0x3000.U)
  val w_valid_address: Bool = (io.w_dmem_add.addr >= 0x2000.U) && (io.w_dmem_add.addr <= 0x3000.U)
  val byteenable: UInt = io.w_dmem_dat.byteenable

  val r_req: Bool = io.r_dmem_add.req && r_valid_address
  val w_req: Bool = io.w_dmem_add.req && w_valid_address

  // initialization
  //val mem: Mem[UInt] = Mem(256*1024, UInt(32.W))
  val mem_3: Mem[UInt] = Mem(1024, UInt(8.W))
  val mem_2: Mem[UInt] = Mem(1024, UInt(8.W))
  val mem_1: Mem[UInt] = Mem(1024, UInt(8.W))
  val mem_0: Mem[UInt] = Mem(1024, UInt(8.W))
  ...
  val addr_align: UInt = Mux(r_req,(io.r_dmem_add.addr >> 2).asUInt() - 0x2000.U,
    Mux(w_req,(io.w_dmem_add.addr >> 2).asUInt() - 0x2000.U,
      0xffffffffL.U))
