...
class DMem extends Module {
  ...
  when(io.r_dmem_add.req === true.B) {
    io.r_dmem_dat.data  := Cat(mem_3.read(io.r_dmem_add.addr),
      mem_2.read(io.r_dmem_add.addr),
      mem_1.read(io.r_dmem_add.addr),
      mem_0.read(io.r_dmem_add.addr)
    )
    i_ack := true.B
    w_ack := false.B
  }.elsewhen(io.w_dmem_add.req === true.B) {
    //mem.write(io.w_dmem_add.addr, io.w_dmem_dat.data)
    when(byteenable(3)){ mem_3.write(io.w_dmem_add.addr, wdat_3) }
    when(byteenable(2)){ mem_2.write(io.w_dmem_add.addr, wdat_2) }
    when(byteenable(1)){ mem_1.write(io.w_dmem_add.addr, wdat_1) }
    when(byteenable(0)){ mem_0.write(io.w_dmem_add.addr, wdat_0) }
    mem_0.write(io.w_dmem_add.addr, wdat_0)
    w_ack := true.B
    i_ack := false.B
  }.otherwise{
    w_ack := false.B
